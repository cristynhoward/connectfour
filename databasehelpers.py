...
def get_outgoing_tweet(tweet_id):
    """ Retrieve and remove game from the collection of outgoing tweets.

    :param tweet_id: The id of the last tweet in the game to be retrieved.
    :type tweet_id: str
    :return: Outgoing game with tweet_id.
    :rtype: dict
    """
    db = pymongo.MongoClient(MY_URI).twitter_bot
    doc = db.Outgoing_Tweets.find_one_and_delete(tweet_id)
...
def load_next_tweet():
    ...
    """
    db = pymongo.MongoClient(MY_URI).twitter_bot
    doc = db.Outgoing_Tweets.find_one_and_delete({"$query": {}, "$orderby": {"_id": +1}})
    # log("Outgoing tweet retrieved and deleted: " + str(doc))
    return doc
...
def get_active_game(tweet_id):
    ...
    """
    db = pymongo.MongoClient(MY_URI).twitter_bot
    doc = db.Active_Games.find_one_and_delete(tweet_id)
